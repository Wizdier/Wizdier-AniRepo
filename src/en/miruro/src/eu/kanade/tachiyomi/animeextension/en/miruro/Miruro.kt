...
class Miruro :
    ...
    private fun browseRequest(page: Int, sort: String, country: String? = null): Request = buildPipeRequest(
        "search/browse",
        "GET",
    ...
    override fun searchAnimeRequest(page: Int, query: String, filters: AnimeFilterList): Request = searchAnimeRequestWithCountry(page, query, filters, resolveCountries(MiruroFilters.getSearchParameters(filters).country).firstOrNull())
        ...
        pairs.forEach { (k, v) ->
            if (v != null) {
                put(
                    k,
                    when (v) {
                        is Int, is Long, is Double, is String, is Boolean -> v
                        else -> v.toString()
                    },
                )
            }
